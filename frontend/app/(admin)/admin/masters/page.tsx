    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 p-6 rounded-3xl backdrop-blur-xl">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">Configure categories, halls, gate locations, and designations</p>
        </div>
        <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl max-w-md w-full space-y-4">
            <h2 className="text-lg font-black text-white">Add Master Item</h2>
            <form onSubmit={handleCreate} className="space-y-3 text-xs">
              <div>
                <label className="text-slate-400 block mb-1 font-bold">Master Type</label>
                <select
                  value={type}
              ...
              <div>
                <label className="text-slate-400 block mb-1 font-bold">Master Name</label>
                <input
                  type="text"
                  required
                  value={name}
              ...
              <div>
                <label className="text-slate-400 block mb-1 font-bold">Code / Alias</label>
                <input
                  type="text"
                  value={code}
            ...
            {loading ? (
              ...
              masters.map((m: any) => (
                <tr key={m.id} className="hover:bg-slate-800/50">
                  <td className="py-4 px-4 text-emerald-400 font-bold">{m.type}</td>
                  <td className="py-4 px-4 font-bold text-white">{m.name}</td>
                  <td className="py-4 px-4 font-mono text-slate-300">{m.code || '—'}</td>
